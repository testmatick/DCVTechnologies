...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:math="http://exslt.org/math"
    ...
    <xsl:template name="htmlHead">
        <head>
            ...
            <meta http-equiv="cache-com.dydacomp.control" content="max-age=0"/>
            <meta http-equiv="cache-com.dydacomp.control" content="no-cache"/>
            <meta http-equiv="cache-com.dydacomp.control" content="no-store"/>
    ...
    <xsl:function name="testng:suiteTestsCount">
        <xsl:param name="testSuiteElements"/>
        <xsl:param name="state"/>
        <xsl:value-of
                select="if ($state = '*') then count($testSuiteElements/test[class[ test-method[not(@is-config)] ]])
                else if ($state = 'FAIL') then count($testSuiteElements/test[class[ test-method[@status='FAIL' and not(@is-config)] ]])
                else if ($state = 'SKIP') then count($testSuiteElements/test[class[ test-method[@status='SKIP' and not(@is-config)] and not(test-method[@status='FAIL' and not(@is-config)]) ]])
                else count($testSuiteElements/test[class[ not(test-method[@status='SKIP' and not(@is-config)]) and not(test-method[@status='FAIL' and not(@is-config)]) ]])"
                />
    </xsl:function>
        ...
        <xsl:result-document href="{testng:absolutePath('overview-chart.svg')}" format="xml">
            <svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid meet" width="{$chartWidth}"
                ...
                <defs>
                    <style type="text/css">
                        <![CDATA[
				            ...
				            .xgrid, .ygrid, .legendtext { font-weight:normal; font-size:18px; font-family:Arial; }
                ...
                <svg id="graphzone" preserveAspectRatio="xMidYMid meet" x="0" y="0">
                    ...
                    <xsl:variable name="testSuiteElements" select="suite"/>

                    <xsl:variable name="failedCount" select="testng:suiteTestsCount($testSuiteElements, 'FAIL')"/>
                    <xsl:variable name="passedCount" select="testng:suiteTestsCount($testSuiteElements, 'PASS')"/>
                    <xsl:variable name="skippedCount" select="testng:suiteTestsCount($testSuiteElements, 'SKIP')"/>
                    <xsl:variable name="totalCount" select="testng:suiteTestsCount($testSuiteElements, '*')"/>
                    ...
                    <text class="legendtext" x="40" y="105">Failed
                        <xsl:value-of select="$failedCount"/> test case(s) (<xsl:value-of select="$failedPercent"/>)
                    </text>
                    <rect style="fill:green;stroke-width:1;stroke:black;" x="10" y="125" width="20" height="20"/>
                    <text class="legendtext" x="40" y="143">Passed
                        <xsl:value-of select="$passedCount"/> test case(s) (<xsl:value-of select="$passPercent"/>)
                    </text>
                    <rect style="fill:yellow;stroke-width:1;stroke:black;" x="10" y="163" width="20" height="20"/>
                    <text class="legendtext" x="40" y="182">Blocked
                        <xsl:value-of select="$skippedCount"/> test case(s) (<xsl:value-of select="$skipPercent"/>)
                    </text>
